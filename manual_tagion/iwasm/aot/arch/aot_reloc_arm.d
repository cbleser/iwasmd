...
enum R_ARM_JMP24 = 29 /* PC relative 24 bit (B/BL<cond>).  */;
...
private SymbolMap[81] target_sym_map = [
    /* clang-format off */
    REG_COMMON_SYMBOLS
...
enum BUILD_TARGET_ARM_DEFAULT = "armv4";
void get_current_target(char* target_buf, uint target_buf_size) {
    ...
    if (strcmp(s, "ARM") == 0) {
        ...
        s_size = BUILD_TARGET_ARM_DEFAULT.sizeof;
    }
...
void init_plt_table(ubyte* plt) {
    ...
    for (i = 0; i < num; i++) {
        ...
        *p++ = 0xe1a00000;
        /* symbol addr */
        *p++ = cast(uint)cast(uintptr_t)target_sym_map[i].symbol_addr;
...
bool apply_relocation(AOTModule* module_, ubyte* target_section_addr, uint target_section_size, ulong reloc_offset, long reloc_addend, uint reloc_type, void* symbol_addr, int symbol_index, char* error_buf, uint error_buf_size) {
    switch (reloc_type) {
        ...
        case R_ARM_JMP24:
        {
            intptr_t result = void;
            int RESULT_MASK = 0x03FFFFFE;
            int insn = *cast(int*)(target_section_addr + reloc_offset);
            /* Initial addend: sign_extend(insn[23:0] << 2) */
            int initial_addend = ((insn & 0xFFFFFF) << 2) | ((insn & 0x800000) ? 0xFC000000 : 0);
            ...
            if (symbol_index < 0) {
                 ...
                 * is 0 */
                result =
                    cast(intptr_t)(cast(uintptr_t)symbol_addr + cast(intptr_t)reloc_addend
                ...
                result = cast(intptr_t)(cast(uintptr_t)plt + cast(intptr_t)reloc_addend
                                    - cast(uintptr_t)(target_section_addr
                                                  + reloc_offset));
            }
            ...
            *cast(int*)(target_section_addr + reloc_offset) =
                (int32)((insn & 0xff000000)
                        | ((cast(int)result & RESULT_MASK) >> 2));
            break;
        }
        case R_ARM_ABS32:
        {
        ...
        case R_ARM_MOVT_ABS:
        {
            ...
            uintptr_t addr = void;
            CHECK_RELOC_OFFSET((void*).sizeof);
            loc = cast(uintptr_t*)(target_section_addr + cast(uint)reloc_offset);
            addr = cast(uintptr_t)symbol_addr + cast(intptr_t)reloc_addend;
