...
static assert(0, "lv_page: lv_cont is required. Enable it in lv_conf.h (LV_USE_CONT  1) ");
}

public import lv_cont;
public import ...lv_core.lv_indev;
...
alias lv_page_edge_t = ubyte;

/*Data of page*/
struct _Lv_page_ext_t {
    ...
    struct _Sb {
        ...
        ubyte hor_draw;/*: 1 !!*/     /*1: horizontal scrollbar is visible now (Handled by the library)*/
        ubyte ver_draw;/*: 1 !!*/     /*1: vertical scrollbar is visible now (Handled by the library)*/
...
static if (LV_USE_ANIMATION) {
    struct _Edge_flash {
        ...
        ubyte enabled;/*: 1 !!*/      /*1: Show a flash animation on the edge*/
        ubyte top_ip;/*: 1 !!*/       /*Used internally to show that top most position is reached (flash is In
                                     Progress)*/
        ubyte bottom_ip;/*: 1 !!*/    /*Used internally to show that bottom most position is reached (flash
                                     is In Progress)*/
        ubyte right_ip;/*: 1 !!*/     /*Used internally to show that right most position is reached (flash
                                     is In Progress)*/
        ubyte left_ip;/*: 1 !!*/      /*Used internally to show that left most position is reached (flash is
                                     In Progress)*/
    }_Edge_flash edge_flash;

    ushort anim_time; /*Scroll animation time*/
}

    ubyte scroll_prop;/*: 1 !!*/    /*1: Propagate the scrolling the the parent if the edge is reached*/
    ubyte scroll_prop_ip;/*: 1 !!*/ /*1: Scroll propagation is in progress (used by the library)*/
}alias lv_page_ext_t = _Lv_page_ext_t;
 ...
 */
ushort lv_page_get_anim_time(const(lv_obj_t)* page);
 ...
 */
void lv_page_set_anim_time(lv_obj_t* page, ushort anim_time);
 ...
 */
pragma(inline, true) private void lv_page_set_scrl_fit4(lv_obj_t* page, lv_fit_t left, lv_fit_t right, lv_fit_t top, lv_fit_t bottom) {
 ...
 */
pragma(inline, true) private void lv_page_set_scrl_fit2(lv_obj_t* page, lv_fit_t hor, lv_fit_t ver) {
 ...
 */
pragma(inline, true) private void lv_page_set_scrl_fit(lv_obj_t* page, lv_fit_t fit) {
 ...
 */
pragma(inline, true) private void lv_page_set_scrl_width(lv_obj_t* page, lv_coord_t w) {
 ...
 */
pragma(inline, true) private void lv_page_set_scrl_height(lv_obj_t* page, lv_coord_t h) {
 ...
 */
pragma(inline, true) private void lv_page_set_scrl_layout(lv_obj_t* page, lv_layout_t layout) {
 ...
 */
pragma(inline, true) private lv_coord_t lv_page_get_scrl_width(const(lv_obj_t)* page) {
 ...
 */
pragma(inline, true) private lv_coord_t lv_page_get_scrl_height(const(lv_obj_t)* page) {
 ...
 */
pragma(inline, true) private lv_layout_t lv_page_get_scrl_layout(const(lv_obj_t)* page) {
 ...
 */
pragma(inline, true) private lv_fit_t lv_page_get_scrl_fit_left(const(lv_obj_t)* page) {
 ...
 * @return an element of `lv_fit_t`
 */
pragma(inline, true) private lv_fit_t lv_page_get_scrl_fit_right(const(lv_obj_t)* page) {
 ...
 * @return an element of `lv_fit_t`
 */
pragma(inline, true) private lv_fit_t lv_page_get_scrl_fit_top(const(lv_obj_t)* page) {
 ...
 * @return an element of `lv_fit_t`
 */
pragma(inline, true) private lv_fit_t lv_page_get_scrl_fit_bottom(const(lv_obj_t)* page) {
