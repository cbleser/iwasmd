...
alias lv_anim_enable_t = ubyte;

/** Type of the animated value*/
alias lv_anim_value_t = lv_coord_t;
...
alias lv_anim_exec_xcb_t = void function(void*, lv_anim_value_t);
...
alias lv_anim_custom_exec_cb_t = void function(_lv_anim_t*, lv_anim_value_t);

/** Get the current value during an animation*/
alias lv_anim_path_cb_t = lv_anim_value_t function(const(_lv_anim_t)*);

/** Callback to call when the animation is ready*/
alias lv_anim_ready_cb_t = void function(_lv_anim_t*);
...
struct _lv_anim_t {
    ...
    int end;                 /**< End value*/
    ushort time;               /**< Animation time in ms*/
    short act_time;            /**< Current time in animation. Set to negative to make delay.*/
    ushort playback_pause;     /**< Wait before play back*/
    ushort repeat_pause;       /**< Wait before repeat*/
static if (LV_USE_USER_DATA) {
    lv_anim_user_data_t user_data; /**< Custom user data*/
}

    ubyte playback;/*: 1 !!*/ /**< When the animation is ready play it back*/
    ubyte repeat;/*: 1 !!*/   /**< Repeat the animation infinitely*/
    /*Animation system use these - user shouldn't set*/
    ubyte playback_now;/*: 1 !!*/ /**< Play back is in progress*/
    uint has_run;/*: 1 !!*/     /**< Indicates the animation has run in this round*/
}alias lv_anim_t = _lv_anim_t;
 ...
 *                E.g. lv_obj_set_x
 */
pragma(inline, true) private void lv_anim_set_exec_cb(lv_anim_t* a, void* var, lv_anim_exec_xcb_t exec_cb) {
    a.var     = var;
    a.exec_cb = exec_cb;
}
 ...
 */
pragma(inline, true) private void lv_anim_set_time(lv_anim_t* a, ushort duration, ushort delay) {
    a.time     = duration;
    a.act_time = -delay;
}
 ...
 */
pragma(inline, true) private void lv_anim_set_values(lv_anim_t* a, lv_anim_value_t start, lv_anim_value_t end) {
    a.start = start;
    a.end   = end;
}
 ...
 * @param exec_cb a function to execute.
 */
pragma(inline, true) private void lv_anim_set_custom_exec_cb(lv_anim_t* a, lv_anim_custom_exec_cb_t exec_cb) {
    a.var     = a;
    a.exec_cb = cast(lv_anim_exec_xcb_t)exec_cb;
}
 ...
 */
pragma(inline, true) private void lv_anim_set_path_cb(lv_anim_t* a, lv_anim_path_cb_t path_cb) {
    a.path_cb = path_cb;
}
 ...
 */
pragma(inline, true) private void lv_anim_set_ready_cb(lv_anim_t* a, lv_anim_ready_cb_t ready_cb) {
    a.ready_cb = ready_cb;
}
 ...
 */
pragma(inline, true) private void lv_anim_set_playback(lv_anim_t* a, ushort wait_time) {
    a.playback       = 1;
 ...
 * @param a pointer to an initialized `lv_anim_t` variable
 */
pragma(inline, true) private void lv_anim_clear_playback(lv_anim_t* a) {
    a.playback = 0;
}
 ...
 */
pragma(inline, true) private void lv_anim_set_repeat(lv_anim_t* a, ushort wait_time) {
    a.repeat       = 1;
 ...
 * @param a pointer to an initialized `lv_anim_t` variable
 */
pragma(inline, true) private void lv_anim_clear_repeat(lv_anim_t* a) {
    a.repeat = 0;
}
 ...
 * @return true: at least 1 animation is deleted, false: no animation is deleted
 */
pragma(inline, true) private bool lv_anim_custom_del(lv_anim_t* a, lv_anim_custom_exec_cb_t exec_cb) {
    return lv_anim_del(a.var, cast(lv_anim_exec_xcb_t)exec_cb);
}
 ...
 */
ushort lv_anim_speed_to_time(ushort speed, lv_anim_value_t start, lv_anim_value_t end);
