...
static assert(0, "lv_kb: lv_btnm is required. Enable it in lv_conf.h (LV_USE_BTNM  1) ");
}

static if (LV_USE_TA == 0) {
static assert(0, "lv_kb: lv_ta is required. Enable it in lv_conf.h (LV_USE_TA  1) ");
}
...
struct _Lv_kb_ext_t {
    ...
    ubyte cursor_mng;/*: 1 !!*/ /*1: automatically show/hide cursor when a text area is assigned or left*/
}alias lv_kb_ext_t = _Lv_kb_ext_t;
 ...
 */
pragma(inline, true) private void lv_kb_set_map(lv_obj_t* kb, const(char)** map) {
 ...
 */
pragma(inline, true) private void lv_kb_set_ctrl_map(lv_obj_t* kb, const(lv_btnm_ctrl_t)* ctrl_map) {
 ...
 */
pragma(inline, true) private const(char)** lv_kb_get_map_array(const(lv_obj_t)* kb) {
