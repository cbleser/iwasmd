 ...
 */
enum REQUES_PACKET_VER = 1;
enum REQUEST_PACKET_FIX_PART_LEN = 18;
enum REQUEST_PACKET_URL_OFFSET = REQUEST_PACKET_FIX_PART_LEN;
enum REQUEST_PACKET_URL_LEN = \
    *((uint16 *)((char *)buffer + 12)) /* to ensure little endian */;
enum REQUEST_PACKET_PAYLOAD_LEN = \
    *((uint32 *)((char *)buffer + 14)) /* to ensure little endian */;
enum string REQUEST_PACKET_URL(string buffer) = ` ((char *)buffer + REQUEST_PACKET_URL_OFFSET)`;
enum string REQUEST_PACKET_PAYLOAD(string buffer) = `          \
    ((char *)buffer + REQUEST_PACKET_URL_OFFSET \
     + REQUEST_PACKET_URL_LEN(buffer))`;

enum RESPONSE_PACKET_FIX_PART_LEN = 16;
...
char* pack_request(request_t* request, int* size) {
    ...
    if ((packet = cast(char*)WA_MALLOC(len)) == null)
        return null;

    /* TODO: ensure little endian for words and dwords */
    *packet = REQUES_PACKET_VER;
    *(cast(ubyte*)(packet + 1)) = request.action;
...
request_t* unpack_request(char* packet, int size, request_t* request) {
    ushort url_len = void, u16 = void;
    uint payload_len = void, u32 = void;
    ...
    request.action = *(cast(ubyte*)(packet + 1));
...
char* pack_response(response_t* response, int* size) {
    ...
    /* TODO: ensure little endian for words and dwords */
    *packet = REQUES_PACKET_VER;
    *(cast(ubyte*)(packet + 1)) = response.status;
...
response_t* unpack_response(char* packet, int size, response_t* response) {
    ...
    response.status = *(cast(ubyte*)(packet + 1));
...
request_t* clone_request(request_t* request) {
    /* deep clone */
    request_t* req = cast(request_t*)WA_MALLOC(request_t.sizeof);
    if (req == null)
        return null;

    memset(req, 0, typeof(*req).sizeof);
    ...
    if (request.payload_len) {
        req.payload = cast(char*)WA_MALLOC(request.payload_len);
        if (!req.payload)
            goto fail;
...
response_t* clone_response(response_t* response) {
    response_t* clone = cast(response_t*)WA_MALLOC(response_t.sizeof);

    if (clone == null)
        return null;

    memset(clone, 0, typeof(*clone).sizeof);
    ...
    if (clone.payload_len) {
        clone.payload = cast(char*)WA_MALLOC(response.payload_len);
        if (!clone.payload)
            goto fail;
...
response_t* set_response(response_t* response, int status, int fmt, const(char)* payload, int payload_len) {
    response.payload = cast(void*)payload;
    response.payload_len = payload_len;
...
request_t* init_request(request_t* request, char* url, int action, int fmt, void* payload, int payload_len) {
    request.url = url;
 ...
 */
char* find_key_value(char* buffer, int buffer_len, char* key, char* value, int value_len, char delimiter) {
    char* p = buffer;
