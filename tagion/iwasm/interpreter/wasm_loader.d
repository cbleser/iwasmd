...
import tagion.iwasm.common.wasm_native;
...
bool CHECK_BUF(const(ubyte)* buf, const(ubyte)* buf_end, uint length) {
    return (!check_buf(buf, buf_end, length, error_buf, error_buf_size));
}
...
private bool is_value_type(ubyte type) {
    if (type == VALUE_TYPE_I32 || type == VALUE_TYPE_I64
            ...
            (type == VALUE_TYPE_FUNCREF || type == VALUE_TYPE_EXTERNREF)
            || (
                ver.WASM_ENABLE_SIMD) &&
            (ver.WASM_ENABLE_WAMR_COMPILER) || (ver.WASM_ENABLE_JIT)
...
static if (ver.WASM_ENABLE_SIMD) {
    static if ((ver.WASM_ENABLE_WAMR_COMPILER) || (ver.WASM_ENABLE_JIT)) {
    ...
    } /* end of (ver.WASM_ENABLE_WAMR_COMPILER) || (ver.WASM_ENABLE_JIT) */
...
private bool load_init_expr(const(ubyte)** p_buf, const(ubyte)* buf_end, InitializerExpression* init_expr, ubyte type, char* error_buf, uint error_buf_size) {
    ...
    ubyte flag = void, end_byte = void;
    ubyte* p_float = void;
    uint i = void;

    if (CHECK_BUF(p, p_end, 1))
        goto fail;
...
private bool load_type_section(const(ubyte)* buf, const(ubyte)* buf_end, WASMModule* module_, char* error_buf, uint error_buf_size) {
    ...
    if (type_count) {
        ...
        for (i = 0; i < type_count; i++) {
            ...
            p_org = p;
            if (CHECK_BUF(p, p_end, param_count))
                goto fail;
            p += param_count;
            read_leb_uint32(p, p_end, result_count);
            if (CHECK_BUF(p, p_end, result_count))
                goto fail;
            p = p_org;
 ...
 */
    private WASMExport* wasm_loader_find_export(const(WASMModule)* module_, const(char)* module_name, const(char)* field_name, ubyte export_kind, char* error_buf, uint error_buf_size) {
        ...
        for (i = 0, export_ = module_.exports; i < module_.export_count; ++i, ++export_) {
            /**
...
private bool load_function_import(const(ubyte)** p_buf, const(ubyte)* buf_end, const(WASMModule)* parent_module, const(char)* sub_module_name, const(
        char)* function_name, WASMFunctionImport* function_, char* error_buf, uint error_buf_size) {
    const(ubyte)* p = *p_buf, p_end = buf_end;
    ...
    *p_buf = p;
    ...
    else static if (ver.WASM_ENABLE_MULTI_MODULE) {
    ...
    function_.module_name = sub_module_name;
    function_.field_name = function_name;
...
private bool load_table_import(const(ubyte)** p_buf, const(ubyte)* buf_end, WASMModule* parent_module, const(char)* sub_module_name, const(
        char)* table_name, WASMTableImport* table, char* error_buf, uint error_buf_size) {
    const(ubyte)* p = *p_buf, p_end = buf_end;
...
private bool load_memory_import(const(ubyte)** p_buf, const(ubyte)* buf_end, WASMModule* parent_module, const(char)* sub_module_name, const(
        char)* memory_name, WASMMemoryImport* memory, char* error_buf, uint error_buf_size) {
    const(ubyte)* p = *p_buf, p_end = buf_end;
    ...
    if (func_count) {
        ...
        for (i = 0; i < func_count; i++) {
            ...
            for (j = 0; j < local_set_count; j++) {
                ...
                if (CHECK_BUF(p_code, buf_code_end, 1))
                    goto fail;
                /* 0x7F/0x7E/0x7D/0x7C */
...
static bool load_data_segment_section(const(ubyte)* buf, const(ubyte)* buf_end,
    ...
    if (data_seg_count) {
        ...
        for (i = 0; i < data_seg_count; i++) {
            ...
            if (!(ver.WASM_ENABLE_BULK_MEMORY) || !is_passive)
            ...
            if (CHECK_BUF(p, p_end, data_seg_len))
                goto fail;
...
static if (ver.WASM_ENABLE_CUSTOM_NAME_SECTION) {
    private bool handle_name_section(const(ubyte)* buf, const(ubyte)* buf_end, WASMModule* module_, bool is_load_from_file_buf, char* error_buf, uint error_buf_size) {
        ...
        while (p < p_end) {
            ...
            if (CHECK_BUF(p, p_end, subsection_size))
                goto fail;
            switch (name_type) {
            case SUB_SECTION_TYPE_FUNC:
                if (subsection_size) {
                    read_leb_uint32(p, p_end, num_func_name);
                    for (name_index = 0; name_index < num_func_name; name_index++) {
                        ...
                        if (CHECK_BUF(p, p_end, func_name_len))
                            goto fail;
    ...
    static if (ver.WASM_ENABLE_LOAD_CUSTOM_SECTION) {
        {
            ...
            section.content_len = cast(uint)(p_end - p - name_len);
...
static if (ver.WASM_ENABLE_FAST_JIT) {
    private bool init_fast_jit_functions(WASMModule* module_, char* error_buf, uint error_buf_size) {
        static if (ver.WASM_ENABLE_LAZY_JIT) {
...
} /* end of ver.WASM_ENABLE_FAST_JIT */
...
static if (ver.WASM_ENABLE_JIT) {
    private bool init_llvm_jit_functions_stage1(WASMModule* module_, char* error_buf, uint error_buf_size) {
        ...
        static if (ver.WASM_ENABLE_FAST_JIT && ver.WASM_ENABLE_LLVM_JIT) {
        ...
        static if ((ver.WASM_ENABLE_PERF_PROFILING) || (ver.WASM_ENABLE_DUMP_CALL_STACK)) {
    ...
    private bool init_llvm_jit_functions_stage2(WASMModule* module_, char* error_buf, uint error_buf_size) {
        ...
        static if (ver.WASM_ENABLE_FAST_JIT && ver.WASM_ENABLE_LAZY_JIT) {
...
static if (ver.WASM_ENABLE_FAST_JIT || ver.WASM_ENABLE_JIT) {
    ...
    private void* orcjit_thread_callback(void* arg) {
        ...
        static if (ver.WASM_ENABLE_FAST_JIT && ver.WASM_ENABLE_JIT
            ...
            for (i = group_idx; i < func_count; i += group_stride * WASM_ORC_JIT_COMPILE_THREAD_NUM) {
                uint j = void;
...
} /* end of ver.WASM_ENABLE_FAST_JIT || ver.WASM_ENABLE_JIT */
...
static if (ver.WASM_ENABLE_FAST_INTERP && ver.WASM_ENABLE_LABELS_AS_VALUES) {
        ...
        if (export_.kind == EXPORT_KIND_GLOBAL) {
            ...
            */
            if (aux_data_end_global && aux_heap_base_global
                ...
                for (global_index = 0; global_index < module_.global_count; global_index++) {
                    global = module_.globals + global_index;
                    if (global.is_mutable /* heap_base and data_end is
                        ...
                        module_.aux_stack_size =
                            ...
                            ? aux_stack_top - aux_data_end : aux_stack_top;
...
private bool create_sections(const(ubyte)* buf, uint size, WASMSection** p_section_list, char* error_buf, uint error_buf_size) {
    ...
    ubyte section_type = void, section_index = void, last_section_index = cast(ubyte)-1;
    ...
    while (p < p_end) {
        ...
        if (section_index != cast(ubyte)-1) {
            if (section_type != SECTION_TYPE_USER) {
                ...
                if (last_section_index != cast(ubyte)-1
            ...
            if (CHECK_BUF1(p, p_end, section_size))
                goto fail;
...
private bool load(const(ubyte)* buf, uint size, WASMModule* module_, char* error_buf, uint error_buf_size) {
    ...
    if (CHECK_BUF1(p, p_end, uint32.sizeof))
        goto fail;
...
static if ((ver.WASM_ENABLE_MULTI_MODULE) && (ver.WASM_ENABLE_LIBC_WASI)) {
    /**
 * refer to
...
WASMModule* wasm_loader_load(ubyte* buf, uint size, bool main_module, char* error_buf, uint error_buf_size) {
    ...
    static if (ver.WASM_ENABLE_DEBUG_INTERP || ver.WASM_ENABLE_FAST_JIT) {
...
void wasm_loader_unload(WASMModule* module_) {
    ...
    static if (ver.WASM_ENABLE_FAST_JIT && WASM_ENABLE_JIT && ver.WASM_ENABLE_LAZY_JIT) {
        module_.orcjit_stop_compiling = true;
        ...
        for (i = 0; i < module_.function_count; i++) {
            if (module_.functions[i]) {
                ...
                static if (ver.WASM_ENABLE_FAST_JIT) {
                    ...
                    static if (ver.WASM_ENABLE_JIT && ver.WASM_ENABLE_LAZY_JIT) {
...
bool wasm_loader_find_block_addr(WASMExecEnv* exec_env, BlockAddr* block_addr_cache, const(ubyte)* start_addr, const(ubyte)* code_end_addr, ubyte label_type, ubyte** p_else_addr, ubyte** p_end_addr) {
    ...
    BlockAddr[16] block_stack = [[0]];
    BlockAddr* block = void;
    ...
    op_break_retry:
        ...
        case DEBUG_OP_BREAK: {
                    ...
                    ulong offset = (p - 1) >= module_inst.module_.load_addr
                        ? (p - 1) - module_inst.module_.load_addr : ~0;
                    if (debug_instance) {
...
static if (ver.WASM_ENABLE_FAST_INTERP) {
    /+
static if (ver.WASM_DEBUG_PREPROCESSOR) {
    ...
    do {                                                                  \
        if (ctx.frame_csp >= ctx.frame_csp_boundary) {                  \
            MEM_REALLOC(                                                  \
                ...
                cast(uint)(ctx.frame_csp_size + 8 * sizeof(BranchBlock))); \
            ctx.frame_csp_size += cast(uint)(8 * sizeof(BranchBlock));     \
...
static if (ver.WASM_ENABLE_FAST_INTERP) {
    private bool check_offset_push(WASMLoaderContext* ctx, char* error_buf, uint error_buf_size) {
        uint cell_num = cast(uint)(ctx.frame_offset - ctx.frame_offset_bottom);
...
static if (ver.WASM_ENABLE_FAST_INTERP) {
    ...
    static if (ver.WASM_ENABLE_LABELS_AS_VALUES) {
        static if (ver.WASM_CPU_SUPPORTS_UNALIGNED_ADDR_ACCESS) {
    ...
    do {                                                                       \
        int32 offset =                                                         \
            cast(int)((uint8 *)handle_table[opcode] - (uint8 *)handle_table[0]); \
    ...
    do {                                            \
        ...
        enum string skip_label() = `                                           \
    do {                                                       \
        wasm_loader_emit_backspace(loader_ctx, ubyte.sizeof); \
        LOG_OP("\ndelete last op\n");                          \
    } while (0)`;
    ...
    private void wasm_loader_emit_const(WASMLoaderContext* ctx, void* value, bool is_32_bit) {
        ...
        if (ctx.p_code_compiled) {
            ...
            bh_memcpy_s(ctx.p_code_compiled,
                    cast(uint)(ctx.p_code_compiled_end - ctx.p_code_compiled),
                    value, size);
    ...
    private bool wasm_loader_get_const_offset(WASMLoaderContext* ctx, ubyte type, void* value, short* offset, char* error_buf, uint error_buf_size) {
        ...
        for (c = cast(Const*) ctx.const_buf; cast(ubyte*) c < ctx.const_buf + ctx.num_const * Const.sizeof; c++) {
...
static if (ver.WASM_ENABLE_FAST_INTERP) {
    ...
    private bool reserve_block_ret(WASMLoaderContext* loader_ctx, ubyte opcode, bool disable_emit, char* error_buf, uint error_buf_size) {
        short operand_offset = 0;
        BranchBlock* block = (opcode == WASM_OP_ELSE) ? loader_ctx.frame_csp - 1 : loader_ctx.frame_csp;
        BlockType* block_type = &block.block_type;
        ubyte* return_types = null;
        uint return_count = 0, value_count = 0, total_cel_num = 0;
        int i = 0;
        short dynamic_offset = void, dynamic_offset_org = void;
        short* frame_offset = null, frame_offset_org = null;
 ...
 */
    private bool copy_params_to_dynamic_space(WASMLoaderContext* loader_ctx, bool is_if_block, char* error_buf, uint error_buf_size) {
        ...
        /* Part b) */
        emit_uint32(loader_ctx, is_if_block ? wasm_type.param_cell_num + 1 : wasm_type.param_cell_num);
        /* Part c) */
        for (i = 0; i < param_count; i++)
...
static bool wasm_loader_prepare_bytecode(WASMModule* module_, WASMFunction* func,
    ...
    ubyte* param_types, local_types;
    ubyte local_type, global_type;
        ...
        case WASM_OP_END: {
                ...
                static if (ver.WASM_ENABLE_FAST_INTERP) {
                    ...
                    if (loader_ctx.frame_csp.label_type == LABEL_TYPE_FUNCTION) {
                        ...
                        for (idx = cast(int) func.func_type.result_count - 1; idx >= 0; idx--) {
        ...
        case WASM_OP_RETURN_CALL:
            }
            {
                ...
                if (func_type.param_count > 0) {
                    for (idx = cast(int)(func_type.param_count - 1); idx >= 0; idx--) {
                ...
                if (!(ver.WASM_ENABLE_TAIL_CALL) || (opcode == WASM_OP_CALL)) {
                    //! #endif
    ...
    }
    {
        ...
        static if (ver.WASM_ENABLE_TAIL_CALL) {
            ...
            static if (ver.WASM_ENABLE_JIT || ver.WASM_ENABLE_WAMR_COMPILER) {
        ...
        case WASM_OP_DROP: {
            ...
            if (available_stack_cell > 0) {
                ...
                else if ((ver.WASM_ENABLE_SIMD) &&
                        ((ver.WASM_ENABLE_WAMR_COMPILER) || (ver.WASM_ENABLE_JIT)) &&
                        (*(loader_ctx.frame_ref - 1) == REF_V128_1)) {
            ...
            else static if (ver.WASM_ENABLE_FAST_INTERP) {
                skip_label();
            }
                            ...
                            static if (ver.WASM_ENABLE_LABELS_AS_VALUES) {
                                static if (ver.WASM_CPU_SUPPORTS_UNALIGNED_ADDR_ACCESS) {
                                    *cast(void**)(p_code_compiled_tmp
                                    ...
                                    int offset = cast(int)(cast(ubyte*) handle_table[opcode_tmp]
        ...
        case WASM_OP_GET_LOCAL: {
                    ...
                    if (local_offset < 0x80) {
                        *p_org++ = EXT_OP_GET_LOCAL_FAST;
                        if (is_32bit_type(local_type)) {
                            ...
                            *p_org++ = cast(ubyte)(local_offset | 0x80);
                        }
                        while (p_org < p) {
                            *p_org++ = WASM_OP_NOP;
                        }
            ...
            } /* end of ver.WASM_ENABLE_FAST_INTERP */
            break;
        }
                 ...
                 */
                    ...
                    case SIMD_f64x2_replace_lane: {
                            ...
                            ubyte[15] push_type = [
                                ...
                                VALUE_TYPE_V128,
                                /*i32x4*/
                                VALUE_TYPE_I32, VALUE_TYPE_V128,
                                /*i64x2*/
                                VALUE_TYPE_I64, VALUE_TYPE_V128,
                                /*f32x4*/
                                VALUE_TYPE_F32, VALUE_TYPE_V128,
                                /*f64x2*/
                                VALUE_TYPE_F64, VALUE_TYPE_V128,
                            ];
                            /* clang-format on */
        ...
        if (loader_ctx.csp_num > 0) {
            if (cur_func_idx < module_.function_count - 1) /* Function with missing end marker (between two functions) */
                set_error_buf(error_buf, error_buf_size, "END opcode expected");
            else /* Function with missing end marker
            ...
            if (func.const_cell_num > 0) {
                ...
                for (j = loader_ctx.num_const - 1; j >= 0; j--) {
                    ...
                    if (c.value_type == VALUE_TYPE_F64
                        ...
                        bh_memcpy_s(func_const, cast(uint)(func_const_end - func_const),
 ...
 */
    ...
    bool wasm_loader_find_block_addr(WASMExecEnv* exec_env, BlockAddr* block_addr_cache, const(ubyte)* start_addr, const(
            ubyte)* code_end_addr, ubyte block_type, ubyte** p_else_addr, ubyte** p_end_addr);
