...
private void set_error_buf(char* error_buf, uint error_buf_size, const(char)* string_) {
    if (error_buf != null) {
        snprintf(error_buf, error_buf_size,
                 "WASM module instantiate failed: %s", string_);
    }
}

private void set_error_buf_v(Args...)(char* error_buf, uint error_buf_size, const(char)* format_, Args args) {
assert(0, "Not translated yet");
	va_list args = void;
    char[128] buf = void;
        ...
        if (init_expr.init_expr_type == INIT_EXPR_TYPE_GET_GLOBAL) {
            ...
            bh_memcpy_s(
                ...
                typeof(globals[init_expr.u.global_index].initial_value).sizeof);
			        ...
			        else if ((ver.WASM_ENABLE_REF_TYPES) && init_expr.init_expr_type == INIT_EXPR_TYPE_REFNULL_CONST) {
...
|| ((ver.WASM_ENABLE_FAST_JIT || ver.WASM_ENABLE_JIT)
        && (module_inst.e.function_count > 0
...
alias	 interp_call_wasm = call_wasm_with_hw_bound_check;
} else {
alias interp_call_wasm = wasm_interp_call_wasm;
}
...
void wasm_dump_perf_profiling(const(WASMModuleInstance)* module_inst) {
    ...
    for (i = 0; i < module_inst.e.function_count; i++) {
        ...
        else if((ver.WASM_ENABLE_CUSTOM_NAME_SECTION) && func_inst.u.func.field_name) {
...
static if (ver.WASM_ENABLE_FAST_INTERP) {
        size +=
            func.code_compiled_size + uint.sizeof * func.const_cell_num;
}
    ...
    for (i = 0; i < module_.table_seg_count; i++) {
        WASMTableSeg* table_seg = &module_.table_segments[i];
        mem_conspn.tables_size += uint.sizeof * table_seg.function_count;
    }
...
} /* end of ver.WASM_ENABLE_BULK_MEMORY */
...
static if (ver.WASM_ENABLE_JIT) {
    ...
    bh_memcpy_s(cast(ubyte*)tbl_inst + WASMTableInstance.elems.offsetof
                ...
                cast(uint)uint.sizeof * (tbl_inst.cur_size - dst_offset),
...
static if (ver.WASM_ENABLE_JIT) {
    ...
    bh_memmove_s(cast(ubyte*)dst_tbl_inst + WASMTableInstance.elems.offsetof
                     + uint.sizeof * dst_offset,
                 cast(uint)uint.sizeof * (dst_tbl_inst.cur_size - dst_offset),
                 cast(ubyte*)src_tbl_inst + WASMTableInstance.elems.offsetof
                     + uint.sizeof * src_offset,
                 cast(uint)uint.sizeof * length);
}
...
} /* end of ver.WASM_ENABLE_REF_TYPES */
...
} /* end of ver.WASM_ENABLE_JIT || ver.WASM_ENABLE_WAMR_COMPILER */
/*
 * Copyright (C) 2019 Intel Corporation.  All rights reserved.
 * SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
 */
...
static if (ver.WASM_ENABLE_FAST_JIT || ver.WASM_ENABLE_JIT 
    || ver.WASM_ENABLE_WAMR_COMPILER || ver.WASM_ENABLE_AOT) {
