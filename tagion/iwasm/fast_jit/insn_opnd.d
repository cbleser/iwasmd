 ...
 */
        /* Basic Block of a label.  */
        if (successful && cc._ann._label_basic_block_enabled) {
            JitBasicBlock* ptr = _jit_realloc(cc._ann._label_basic_block, JitBasicBlock.sizeof * capacity, JitBasicBlock.sizeof * num);
            if (ptr)
                cc._ann._label_basic_block = ptr;
            else
                successful = false;
        }
        /* Predecessor number of the block that is only used in
   jit_cc_update_cfg for updating the CFG.  */
        if (successful && cc._ann._label_pred_num_enabled) {
            uint16* ptr = _jit_realloc(cc._ann._label_pred_num, uint16.sizeof * capacity, uint16.sizeof * num);
            if (ptr)
                cc._ann._label_pred_num = ptr;
            else
                successful = false;
        }
        /* Execution frequency of a block.  We can split critical edges with
   empty blocks so we don't need to store frequencies of edges.  */
        if (successful && cc._ann._label_freq_enabled) {
            uint16* ptr = _jit_realloc(cc._ann._label_freq, uint16.sizeof * capacity, uint16.sizeof * num);
            if (ptr)
                cc._ann._label_freq = ptr;
            else
                successful = false;
        }
        /* Begin bytecode instruction pointer of the block.  */
        if (successful && cc._ann._label_begin_bcip_enabled) {
            uint8 *  * ptr = _jit_realloc(cc._ann._label_begin_bcip, (uint8*).sizeof.sizeof * capacity, (uint8*).sizeof * num);
            if (ptr)
                cc._ann._label_begin_bcip = ptr;
            else
                successful = false;
        }
        /* End bytecode instruction pointer of the block.  */
        if (successful && cc._ann._label_end_bcip_enabled) {
            uint8 *  * ptr = _jit_realloc(cc._ann._label_end_bcip, (uint8*).sizeof * capacity, (uint8*).sizeof * num);
            if (ptr)
                cc._ann._label_end_bcip = ptr;
            else
                successful = false;
        }
        /* Stack pointer offset at the end of the block.  */
        if (successful && cc._ann._label_end_sp_enabled) {
            uint16* ptr = _jit_realloc(cc._ann._label_end_sp, uint16.sizeof * capacity, uint16.sizeof * num);
            if (ptr)
                cc._ann._label_end_sp = ptr;
            else
                successful = false;
        }
        /* The label of the next physically adjacent block.  */
        if (successful && cc._ann._label_next_label_enabled) {
            JitReg* ptr = _jit_realloc(cc._ann._label_next_label, JitReg.sizeof * capacity, JitReg.sizeof * num);
            if (ptr)
                cc._ann._label_next_label = ptr;
            else
                successful = false;
        }
        /* Compiled code address of the block.  */
        if (successful && cc._ann._label_jitted_addr_enabled) {
            void *  * ptr = _jit_realloc(cc._ann._label_jitted_addr, (void*).sizeof * capacity, (void*).sizeof * num);
            if (ptr)
                cc._ann._label_jitted_addr = ptr;
            else
                successful = false;
        }
        /**
 ...
 */
   ...
   value, which is only used by the compilation context's hash table
   of instructions.  */
            if (successful && cc._ann._insn__hash_link_enabled) {
                JitInsn* ptr = _jit_realloc(cc._ann._insn__hash_link, JitInsn.sizeof * capacity, JitInsn.sizeof * num);
                if (ptr)
                    cc._ann._insn__hash_link = ptr;
                else
                    successful = false;
            }
            /**
...
JitReg jit_cc_new_reg(JitCompContext* cc, uint kind) {
    ...
    if (num == capacity) {
        capacity = (capacity == 0/* Initialize the capacity to be larger than hard
                           register number.  */
                ? cc.hreg_info.info[kind].num + 16 : capacity + capacity / 2);
        /*
 * Copyright (C) 2021 Intel Corporation.  All rights reserved.
 * SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
 */
 ...
 */
        /* Defining instruction of registers satisfying SSA property.  */
        if (successful && cc._ann._reg_def_insn_enabled) {
            JitInsn* ptr = _jit_realloc(cc._ann._reg_def_insn[kind], JitInsn.sizeof * capacity, JitInsn.sizeof * num);
            if (ptr)
                cc._ann._reg_def_insn[kind] = ptr;
            else
                successful = false;
        }
        if (!successful) {
...
bool jit_annl_enable_basic_block(JitCompContext* cc) {
    if (cc._ann._label_basic_block_enabled)
        return true;
...
bool jit_annl_enable_begin_bcip(JitCompContext* cc) {
    if (cc._ann._label_begin_bcip_enabled)
        return true;
    if (cc._ann._label_capacity > 0 && ((cc._ann._label_begin_bcip = jit_calloc(cc._ann._label_capacity * (ubyte*).sizeof)) == 0)) {
...
bool jit_annl_enable_end_bcip(JitCompContext* cc) {
    if (cc._ann._label_end_bcip_enabled)
        return true;
    if (cc._ann._label_capacity > 0 && ((cc._ann._label_end_bcip = jit_calloc(cc._ann._label_capacity * (ubyte*).sizeof)) == 0)) {
...
bool jit_annl_enable_next_label(JitCompContext* cc) {
    if (cc._ann._label_next_label_enabled)
        return true;
    if (cc._ann._label_capacity > 0 && ((cc._ann._label_next_label = jit_calloc(cc._ann._label_capacity * JitReg.sizeof)) == 0)) {
...
bool jit_annl_enable_jitted_addr(JitCompContext* cc) {
    if (cc._ann._label_jitted_addr_enabled)
        return true;
    if (cc._ann._label_capacity > 0 && ((cc._ann._label_jitted_addr = jit_calloc(cc._ann._label_capacity * (void*).sizeof)) == 0)) {
...
bool jit_anni_enable__hash_link(JitCompContext* cc) {
    if (cc._ann._insn__hash_link_enabled)
        return true;
    if (cc._ann._insn_capacity > 0 && ((cc._ann._insn__hash_link = jit_calloc(cc._ann._insn_capacity * (JitInsn*).sizeof)) == 0)) {
...
bool jit_annr_enable_def_insn(JitCompContext* cc) {
    uint k = void;
    if (cc._ann._reg_def_insn_enabled)
        return true;
...
void jit_annl_disable_basic_block(JitCompContext* cc) {
    jit_free(cc._ann._label_basic_block);
    cc._ann._label_basic_block = null;
    cc._ann._label_basic_block_enabled = 0;
}
/* Predecessor number of the block that is only used in
   jit_cc_update_cfg for updating the CFG.  */
void jit_annl_disable_pred_num(JitCompContext* cc) {
    jit_free(cc._ann._label_pred_num);
    cc._ann._label_pred_num = null;
    cc._ann._label_pred_num_enabled = 0;
}
/* Execution frequency of a block.  We can split critical edges with
   empty blocks so we don't need to store frequencies of edges.  */
void jit_annl_disable_freq(JitCompContext* cc) {
    jit_free(cc._ann._label_freq);
    cc._ann._label_freq = null;
    cc._ann._label_freq_enabled = 0;
}
/* Begin bytecode instruction pointer of the block.  */
void jit_annl_disable_begin_bcip(JitCompContext* cc) {
    jit_free(cc._ann._label_begin_bcip);
    cc._ann._label_begin_bcip = null;
    cc._ann._label_begin_bcip_enabled = 0;
}
/* End bytecode instruction pointer of the block.  */
void jit_annl_disable_end_bcip(JitCompContext* cc) {
    jit_free(cc._ann._label_end_bcip);
    cc._ann._label_end_bcip = null;
    cc._ann._label_end_bcip_enabled = 0;
}
/* Stack pointer offset at the end of the block.  */
void jit_annl_disable_end_sp(JitCompContext* cc) {
    jit_free(cc._ann._label_end_sp);
    cc._ann._label_end_sp = null;
    cc._ann._label_end_sp_enabled = 0;
}
/* The label of the next physically adjacent block.  */
void jit_annl_disable_next_label(JitCompContext* cc) {
    jit_free(cc._ann._label_next_label);
    cc._ann._label_next_label = null;
    cc._ann._label_next_label_enabled = 0;
}
/* Compiled code address of the block.  */
void jit_annl_disable_jitted_addr(JitCompContext* cc) {
    jit_free(cc._ann._label_jitted_addr);
    cc._ann._label_jitted_addr = null;
    cc._ann._label_jitted_addr_enabled = 0;
}
/**
 * @def ANN_INSN (TYPE, NAME)
 *
...
void jit_anni_disable__hash_link(JitCompContext* cc) {
    jit_free(cc._ann._insn__hash_link);
    cc._ann._insn__hash_link = null;
    cc._ann._insn__hash_link_enabled = 0;
}
/**
 * @def ANN_REG (TYPE, NAME)
 *
...
void jit_annr_disable_def_insn(JitCompContext* cc) {
    uint k = void;
    for (k = JIT_REG_KIND_VOID; k < JIT_REG_KIND_L32; k++) {
        jit_free(cc._ann._reg_def_insn[k]);
        cc._ann._reg_def_insn[k] = null;
    }
    cc._ann._reg_def_insn_enabled = 0;
}
