...
static assert(0, "lv_btn: lv_cont is required. Enable it in lv_conf.h (LV_USE_CONT  1) ");
}
...
alias lv_btn_state_t = ubyte;

/** Extended data of button*/
struct _Lv_btn_ext_t {
    /** Ext. of ancestor*/
    lv_cont_ext_t cont;
    ...
    const(lv_style_t)*[_LV_BTN_STATE_NUM] styles;
static if (LV_BTN_INK_EFFECT) {
    /** [ms] Time of ink fill effect (0: disable ink effect)*/
    ushort ink_in_time;

    /** [ms] Wait before the ink disappears */
    ushort ink_wait_time;
    ...
    /** 1: Toggle enabled*/
    ubyte toggle;/*: 1 !!*/
}alias lv_btn_ext_t = _Lv_btn_ext_t;
 ...
 */
pragma(inline, true) private void lv_btn_set_layout(lv_obj_t* btn, lv_layout_t layout) {
 ...
 */
pragma(inline, true) private void lv_btn_set_fit4(lv_obj_t* btn, lv_fit_t left, lv_fit_t right, lv_fit_t top, lv_fit_t bottom) {
 ...
 */
pragma(inline, true) private void lv_btn_set_fit2(lv_obj_t* btn, lv_fit_t hor, lv_fit_t ver) {
 ...
 */
pragma(inline, true) private void lv_btn_set_fit(lv_obj_t* btn, lv_fit_t fit) {
 ...
 */
pragma(inline, true) private lv_layout_t lv_btn_get_layout(const(lv_obj_t)* btn) {
 ...
 */
pragma(inline, true) private lv_fit_t lv_btn_get_fit_left(const(lv_obj_t)* btn) {
 ...
 * @return an element of `lv_fit_t`
 */
pragma(inline, true) private lv_fit_t lv_btn_get_fit_right(const(lv_obj_t)* btn) {
 ...
 * @return an element of `lv_fit_t`
 */
pragma(inline, true) private lv_fit_t lv_btn_get_fit_top(const(lv_obj_t)* btn) {
 ...
 * @return an element of `lv_fit_t`
 */
pragma(inline, true) private lv_fit_t lv_btn_get_fit_bottom(const(lv_obj_t)* btn) {
