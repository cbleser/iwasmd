...
static assert(0, "lv_win: lv_btn is required. Enable it in lv_conf.h (LV_USE_BTN  1) ");
}

static if (LV_USE_LABEL == 0) {
static assert(0, "lv_win: lv_label is required. Enable it in lv_conf.h (LV_USE_LABEL  1) ");
}

static if (LV_USE_IMG == 0) {
static assert(0, "lv_win: lv_img is required. Enable it in lv_conf.h (LV_USE_IMG  1) ");
}

static if (LV_USE_PAGE == 0) {
static assert(0, "lv_win: lv_page is required. Enable it in lv_conf.h (LV_USE_PAGE  1) ");
}
...
}alias lv_win_ext_t = _Lv_win_ext_t;
 ...
 */
void lv_win_set_anim_time(lv_obj_t* win, ushort anim_time);
 ...
 */
ushort lv_win_get_anim_time(const(lv_obj_t)* win);
 ...
 */
pragma(inline, true) private bool lv_win_get_drag(const(lv_obj_t)* win) {
 ...
 */
pragma(inline, true) private void lv_win_scroll_hor(lv_obj_t* win, lv_coord_t dist) {
    lv_win_ext_t* ext = cast(lv_win_ext_t*)lv_obj_get_ext_attr(win);
 ...
 */
pragma(inline, true) private void lv_win_scroll_ver(lv_obj_t* win, lv_coord_t dist) {
    lv_win_ext_t* ext = cast(lv_win_ext_t*)lv_obj_get_ext_attr(win);
