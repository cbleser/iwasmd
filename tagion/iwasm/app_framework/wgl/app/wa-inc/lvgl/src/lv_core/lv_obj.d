...
static if (LV_HOR_RES_MAX == 0 || LV_VER_RES_MAX == 0) {
static assert(0, "LittlevGL: LV_HOR_RES_MAX and LV_VER_RES_MAX must be greater than 0");
}

static if (LV_ANTIALIAS > 1) {
static assert(0, "LittlevGL: LV_ANTIALIAS can be only 0 or 1");
}

enum LV_MAX_ANCESTOR_NUM = 8;

enum LV_EXT_CLICK_AREA_OFF = 0;
enum LV_EXT_CLICK_AREA_TINY = 1;
enum LV_EXT_CLICK_AREA_FULL = 2;
 ...
 */
alias lv_design_cb_t = bool function(_lv_obj_t* obj, const(lv_area_t)* mask_p, lv_design_mode_t mode);
...
alias lv_event_t = ubyte; /* Type of event being sent to the object. */
 ...
 */
alias lv_event_cb_t = void function(_lv_obj_t* obj, lv_event_t event);
...
alias lv_signal_t = ubyte;

alias lv_signal_cb_t = lv_res_t function(_lv_obj_t* obj, lv_signal_t sign, void* param);
...
alias lv_align_t = ubyte;

static if (LV_USE_OBJ_REALIGN) {
struct _Lv_reailgn_t {
    const(_lv_obj_t)* base;
    lv_coord_t xofs;
    lv_coord_t yofs;
    lv_align_t align_;
    ubyte auto_realign;/*: 1 !!*/
    ubyte origo_align;/*: 1 !!*/ /* 1: the origo (center of the object) was
                                aligned with `lv_obj_align_origo`*/
}alias lv_reailgn_t = _Lv_reailgn_t;
}
...
alias lv_drag_dir_t = ubyte;

alias lv_obj_t = void;
alias lv_obj_type_t = void;
 ...
 * @param y_mod y coordinate shift after alignment
 */
void lv_obj_align_origo(lv_obj_t* obj, const(lv_obj_t)* base, lv_align_t align_, lv_coord_t x_mod, lv_coord_t y_mod);
 ...
 */
void lv_obj_set_protect(lv_obj_t* obj, ubyte prot);
 ...
 * @param prot 'OR'-ed values from `lv_protect_t`
 */
void lv_obj_clear_protect(lv_obj_t* obj, ubyte prot);
 ...
 */
void* lv_obj_allocate_ext_attr(lv_obj_t* obj, ushort ext_size);
 ...
 */
ushort lv_obj_count_children(const(lv_obj_t)* obj);

/** Recursively count the children of an object
 * @param obj pointer to an object
 * @return children number of 'obj'
 */
ushort lv_obj_count_children_recursive(const(lv_obj_t)* obj);
 ...
 */
ubyte lv_obj_get_protect(const(lv_obj_t)* obj);
 ...
 */
bool lv_obj_is_protected(const(lv_obj_t)* obj, ubyte prot);
 ...
 * }
 */
enum string LV_EVENT_CB_DECLARE(string name) = ` void name(lv_obj_t *obj, lv_event_t e)`;
