...
static assert(0, "lv_ta: lv_page is required. Enable it in lv_conf.h (LV_USE_PAGE  1) ");
}

static if (LV_USE_LABEL == 0) {
static assert(0, "lv_ta: lv_label is required. Enable it in lv_conf.h (LV_USE_LABEL  1) ");
}
 ...
 *      DEFINES
 *********************/
enum LV_TA_CURSOR_LAST = (0x7FFF) /*Put the cursor after the last character*/;
...
struct _Lv_ta_ext_t {
    ...
    ushort max_length;         /*The max. number of characters. 0: no limit*/
    ushort pwd_show_time;      /*Time to show characters in password mode before change them to '*' */
    struct _Cursor {
        ...
        lv_coord_t valid_x;        /* Used when stepping up/down to a shorter line.
                                    * (Used by the library)*/
        ushort pos;              /* The current cursor position
                                    * (0: before 1st letter; 1: before 2nd letter ...)*/
        ushort blink_time;       /*Blink period*/
        lv_area_t area;            /* Cursor area relative to the Text Area*/
        ushort txt_byte_pos;     /* Byte index of the letter after (on) the cursor*/
        lv_cursor_type_t type;/*: 4 !!*/ /* Shape of the cursor*/
        ubyte state;/*: 1 !!*/         /*Cursor is visible now or not (Handled by the library)*/
        ubyte click_pos;/*: 1 !!*/     /*1: Enable positioning the cursor by clicking the text area*/
    }_Cursor cursor;
static if (LV_LABEL_TEXT_SEL) {
    ushort tmp_sel_start;       /*Temporary value*/
    ushort tmp_sel_end;         /*Temporary value*/
    ubyte text_sel_in_prog;/*: 1 !!*/ /*User is in process of selecting */
    ubyte text_sel_en;/*: 1 !!*/      /*Text can be selected on this text area*/
}
    ubyte pwd_mode;/*: 1 !!*/ /*Replace characters with '*' */
    ubyte one_line;/*: 1 !!*/ /*One line mode (ignore line breaks)*/
}alias lv_ta_ext_t = _Lv_ta_ext_t;
 ...
 */
void lv_ta_add_char(lv_obj_t* ta, uint c);
 ...
 */
void lv_ta_set_cursor_pos(lv_obj_t* ta, short pos);
 ...
 */
void lv_ta_set_text_align(lv_obj_t* ta, lv_label_align_t align_);
 ...
 */
void lv_ta_set_max_length(lv_obj_t* ta, ushort num);
 ...
 */
pragma(inline, true) private void lv_ta_set_sb_mode(lv_obj_t* ta, lv_sb_mode_t mode) {
 ...
 */
pragma(inline, true) private void lv_ta_set_scroll_propagation(lv_obj_t* ta, bool en) {
 ...
 */
pragma(inline, true) private void lv_ta_set_edge_flash(lv_obj_t* ta, bool en) {
 ...
 */
void lv_ta_set_pwd_show_time(lv_obj_t* ta, ushort time);
 ...
 */
void lv_ta_set_cursor_blink_time(lv_obj_t* ta, ushort time);
 ...
 */
ushort lv_ta_get_cursor_pos(const(lv_obj_t)* ta);
 ...
 */
ushort lv_ta_get_max_length(lv_obj_t* ta);
 ...
 */
pragma(inline, true) private lv_sb_mode_t lv_ta_get_sb_mode(const(lv_obj_t)* ta) {
 ...
 */
pragma(inline, true) private bool lv_ta_get_scroll_propagation(lv_obj_t* ta) {
 ...
 * @return true or false
 */
pragma(inline, true) private bool lv_ta_get_edge_flash(lv_obj_t* ta) {
 ...
 */
ushort lv_ta_get_pwd_show_time(lv_obj_t* ta);
 ...
 */
ushort lv_ta_get_cursor_blink_time(lv_obj_t* ta);
