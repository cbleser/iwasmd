...
enum LV_CHART_POINT_DEF = (LV_COORD_MIN);

/**Automatically calculate the tick length*/
enum LV_CHART_TICK_LENGTH_AUTO = 255;
...
alias lv_chart_update_mode_t = ubyte;

struct _Lv_chart_series_t {
    lv_coord_t* points;
    lv_color_t color;
    ushort start_point;
}alias lv_chart_series_t = _Lv_chart_series_t;
...
alias lv_chart_axis_options_t = ubyte;

struct _Lv_chart_axis_cfg_t {
    const(char)* list_of_values;
    lv_chart_axis_options_t options;
    ubyte num_tick_marks;
    ubyte major_tick_len;
    ubyte minor_tick_len;
}alias lv_chart_axis_cfg_t = _Lv_chart_axis_cfg_t;

/*Data of chart */
struct _Lv_chart_ext_t {
    ...
    ubyte hdiv_cnt;     /*Number of horizontal division lines*/
    ubyte vdiv_cnt;     /*Number of vertical division lines*/
    ushort point_cnt;   /*Point number in a data line*/
    ...
    struct _Series {
        ...
        ubyte num;      /*Number of data lines in dl_ll*/
        lv_opa_t opa;     /*Opacity of data lines*/
        lv_opa_t dark;    /*Dark level of the point/column bottoms*/
    }_Series series;
}alias lv_chart_ext_t = _Lv_chart_ext_t;

enum {
    LV_CHART_STYLE_MAIN,
};
alias lv_chart_style_t = ubyte;
 ...
 */
void lv_chart_set_div_line_count(lv_obj_t* chart, ubyte hdiv, ubyte vdiv);
 ...
 */
void lv_chart_set_point_count(lv_obj_t* chart, ushort point_cnt);
 ...
 */
pragma(inline, true) private void lv_chart_set_style(lv_obj_t* chart, lv_chart_style_t type, const(lv_style_t)* style) {
 ...
 */
void lv_chart_set_x_tick_length(lv_obj_t* chart, ubyte major_tick_len, ubyte minor_tick_len);
 ...
 *                       (where no labels are added)
 */
void lv_chart_set_y_tick_length(lv_obj_t* chart, ubyte major_tick_len, ubyte minor_tick_len);
 ...
 */
void lv_chart_set_x_tick_texts(lv_obj_t* chart, const(char)* list_of_values, ubyte num_tick_marks, lv_chart_axis_options_t options);
 ...
 * @param options           extra options
 */
void lv_chart_set_y_tick_texts(lv_obj_t* chart, const(char)* list_of_values, ubyte num_tick_marks, lv_chart_axis_options_t options);
 ...
 */
void lv_chart_set_margin(lv_obj_t* chart, ushort margin);
 ...
 */
ushort lv_chart_get_point_cnt(const(lv_obj_t)* chart);
 ...
 */
pragma(inline, true) private const(lv_style_t)* lv_chart_get_style(const(lv_obj_t)* chart, lv_chart_style_t type) {
    cast(void)type; /*Unused*/
 ...
 */
ushort lv_chart_get_margin(lv_obj_t* chart);
