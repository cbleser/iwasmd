...
alias PacketHandler = void function(WASMGDBServer* server, char* payload);
...
enum string DEL_HANDLER(string r, string h) = ` [r] = { .request = r, .handler = h }`;

private const(packet_handler_elem)[255] packet_handler_table = [
...
WASMGDBServer* wasm_create_gdbserver(const(char)* host, int* port) {
    bh_socket_t listen_fd = (bh_socket_t)-1;
    WASMGDBServer* server = void;

    bh_assert(port);

    if (((server = wasm_runtime_malloc(WASMGDBServer.sizeof)) == 0)) {
        LOG_ERROR("wasm gdb server error: failed to allocate memory");
        return null;
    }

    memset(server, 0, WASMGDBServer.sizeof);

    if (((server.receive_ctx =
              wasm_runtime_malloc(rsp_recv_context_t.sizeof)) == 0)) {
        LOG_ERROR("wasm gdb server error: failed to allocate memory");
        goto fail;
    }

    memset(server.receive_ctx, 0, rsp_recv_context_t.sizeof);
...
bool wasm_gdbserver_listen(WASMGDBServer* server) {
    int ret = void;
...
bool wasm_gdbserver_accept(WASMGDBServer* server) {
...
void wasm_gdbserver_detach(WASMGDBServer* server) {
...
void wasm_close_gdbserver(WASMGDBServer* server) {
    if (server.receive_ctx) {
        wasm_runtime_free(server.receive_ctx);
    }
    if (server.socket_fd > 0) {
...
pragma(inline, true) private void handle_packet(WASMGDBServer* server, char request, char* payload) {
    if (packet_handler_table[cast(int)request].handler != null)
        packet_handler_table[cast(int)request].handler(server, payload);
}

private void process_packet(WASMGDBServer* server) {
    ubyte* inbuf = cast(ubyte*)server.receive_ctx.receive_buffer;
    char request = void;
    char* payload = null;
    ...
    payload = cast(char*)&inbuf[1];

    LOG_VERBOSE("receive request:%c %s\n", request, payload);
    handle_packet(server, request, payload);
}

pragma(inline, true) private void push_byte(rsp_recv_context_t* ctx, ubyte ch, bool checksum) {
    if (ctx.receive_index >= typeof(ctx.receive_buffer).sizeof) {
 ...
 */
private int on_rsp_byte_arrive(ubyte ch, rsp_recv_context_t* ctx) {
...
bool wasm_gdbserver_handle_packet(WASMGDBServer* server) {
    int n = void;
    char[1024] buf = void;
    ...
    n = os_socket_recv(server.socket_fd, buf.ptr, buf.sizeof);
...
version (BH_PLATFORM_WINDOWS) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (ret == 1) {
                if (!server.noack)
                    write_data_raw(server, cast(ubyte*)"+", 1);
...
alias eStoppointInvalid = GDBStoppointType.eStoppointInvalid;
alias eBreakpointSoftware = GDBStoppointType.eBreakpointSoftware;
alias eBreakpointHardware = GDBStoppointType.eBreakpointHardware;
alias eWatchpointWrite = GDBStoppointType.eWatchpointWrite;
alias eWatchpointRead = GDBStoppointType.eWatchpointRead;
alias eWatchpointReadWrite = GDBStoppointType.eWatchpointReadWrite;


enum rsp_recv_phase_t {
    Phase_Idle,
    Phase_Payload,
    Phase_Checksum
}
alias Phase_Idle = rsp_recv_phase_t.Phase_Idle;
alias Phase_Payload = rsp_recv_phase_t.Phase_Payload;
alias Phase_Checksum = rsp_recv_phase_t.Phase_Checksum;


/* Remote Serial Protocol Receive Context */
struct rsp_recv_context_t {
    rsp_recv_phase_t phase = void;
    ushort receive_index = void;
    ushort size_in_phase = void;
    ubyte check_sum = void;
    /* RSP packet should not be too long */
    char[1024] receive_buffer = void;
}

struct WasmDebugPacket {
    ubyte[PACKET_BUF_SIZE] buf = void;
    uint size = void;
}

struct WASMDebugControlThread;
struct WASMGDBServer {
    bh_socket_t listen_fd = void;
    bh_socket_t socket_fd = void;
    WasmDebugPacket pkt = void;
    bool noack = void;
    WASMDebugControlThread* thread = void;
    rsp_recv_context_t* receive_ctx = void;
}
