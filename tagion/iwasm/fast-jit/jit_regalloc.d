...
private bool uint_stack_push(UintStack** stack, uint val) {
    uint capacity = *stack ? (*stack).capacity : 0;
    uint top = *stack ? (*stack).top : 0;

    bh_assert(top <= capacity);

    if (top == capacity) {
        const(uint) elem_size = typeof((*stack).elem[0]).sizeof;
        uint new_capacity = capacity ? capacity + capacity / 2 : 4;
        UintStack* new_stack = jit_malloc(UintStack.elem.offsetof + elem_size * new_capacity);
...
private int uint_stack_top(UintStack* stack) {
    return stack.elem[stack.top - 1];
}

private void uint_stack_delete(UintStack** stack) {
    jit_free(*stack);
    *stack = null;
}

private void uint_stack_pop(UintStack** stack) {
 ...
 */
private VirtualReg* rc_get_vr(RegallocContext* rc, JitReg vreg) {
    uint kind = jit_reg_kind(vreg);
    uint no = jit_reg_no(vreg);
 ...
 */
private HardReg* rc_get_hr(RegallocContext* rc, JitReg hreg) {
    uint kind = jit_reg_kind(hreg);
    uint no = jit_reg_no(hreg);
 ...
 */
private SpillSlot* rc_get_spill_slot(RegallocContext* rc, JitReg slot) {
    uint index = jit_cc_get_const_I32(rc.cc, slot);
 ...
 */
private uint get_reg_stride(JitReg reg) {
    static const(ubyte)[9] strides = [ 0, 1, 2, 1, 2, 2, 4, 8, 0 ];
 ...
 */
private JitReg rc_alloc_spill_slot(RegallocContext* rc, JitReg vreg) {
    const(uint) stride = get_reg_stride(vreg);
    uint mask = void, new_num = void, i = void, j = void;
    SpillSlot* slots = void;
    ...
    if (((slots = jit_calloc(sizeof(*slots) * new_num)) == 0))
        return 0;
 ...
 */
private void rc_free_spill_slot(RegallocContext* rc, JitReg slot_reg) {
    if (slot_reg) {
...
private bool rc_init(RegallocContext* rc, JitCompContext* cc) {
    uint i = void, j = void;
    ...
    for (i = JIT_REG_KIND_VOID; i < JIT_REG_KIND_L32; i++) {
        ...
        if (vreg_num > 0
            && ((rc.vregs[i] = jit_calloc(sizeof(VirtualReg) * vreg_num)) == 0))
            goto fail;
        if (hreg_num > 0
            && ((rc.hregs[i] = jit_calloc(sizeof(HardReg) * hreg_num)) == 0))
            goto fail;
 ...
 */
private bool is_alloc_candidate(JitCompContext* cc, JitReg reg) {
    return (jit_reg_is_variable(reg)
            && (!jit_cc_is_hreg(cc, reg) || !jit_cc_is_hreg_fixed(cc, reg)));
}

version (VREG_DEF_SANITIZER) {
private void check_vreg_definition(RegallocContext* rc, JitInsn* insn) {
    JitRegVec regvec = jit_insn_opnd_regs(insn);
    JitReg* regp = void; JitReg reg_defined = 0;
    uint i = void, first_use = jit_insn_opnd_first_use(insn);
 ...
 */
private int collect_distances(RegallocContext* rc, JitBasicBlock* basic_block) {
    JitInsn* insn = void;
    int distance = 1;
...
private JitReg offset_of_spill_slot(JitCompContext* cc, JitReg slot) {
 ...
 */
private JitInsn* reload_vreg(RegallocContext* rc, JitReg vreg, JitInsn* cur_insn) {
    ...
    {
        JitReg fp_reg = rc.cc.fp_reg, offset = void;

        if (!vr.slot && ((vr.slot = rc_alloc_spill_slot(rc, vreg)) == 0))
 ...
 */
private JitInsn* spill_vreg(RegallocContext* rc, JitReg vreg, JitInsn* cur_insn) {
    VirtualReg* vr = rc_get_vr(rc, vreg);
    JitReg fp_reg = rc.cc.fp_reg, offset = void;
    JitInsn* insn = void;
 ...
 */
private JitReg allocate_hreg(RegallocContext* rc, JitReg vreg, JitInsn* insn, int distance) {
    const(int) kind = jit_reg_kind(vreg);
    const(HardReg)* hregs = rc.hregs[kind];
    const(uint) hreg_num = jit_cc_hreg_num(rc.cc, kind);
    JitReg hreg = void, vreg_to_reload = 0;
    int min_distance = distance, vr_distance = void;
    VirtualReg* vr = rc_get_vr(rc, vreg);
    uint i = void;
 ...
 */
private JitReg allocate_for_vreg(RegallocContext* rc, JitReg vreg, JitInsn* insn, int distance) {
    VirtualReg* vr = rc_get_vr(rc, vreg);
...
private bool clobber_live_regs(RegallocContext* rc, bool is_native, JitInsn* insn) {
    ...
    for (i = JIT_REG_KIND_VOID; i < JIT_REG_KIND_L32; i++) {
        ...
        for (j = 0; j < hreg_num; j++) {
            ...
            bool caller_saved = (is_native ? jit_cc_is_hreg_caller_saved_native(rc.cc, hreg)
 ...
 * @return true if succeeds, false otherwise
 */
private bool allocate_for_basic_block(RegallocContext* rc, JitBasicBlock* basic_block, int distance) {
    JitInsn* insn = void;

    JIT_FOREACH_INSN_REVERSE(basic_block, insn)
    {
        JitRegVec regvec = jit_insn_opnd_regs(insn);
        uint first_use = jit_insn_opnd_first_use(insn);
        uint i = void;
        JitReg* regp = void;
        ...
        if (is_alloc_candidate(rc.cc, *regp)) {
            ...
            if (((*regp = allocate_for_vreg(rc, vreg, insn, distance)) == 0))
                return false;
...
bool jit_pass_regalloc(JitCompContext* cc) {
    ...
    uint label_index = void, end_label_index = void;
    JitBasicBlock* basic_block = void;
    VirtualReg* self_vr = void;
    bool retval = false;
