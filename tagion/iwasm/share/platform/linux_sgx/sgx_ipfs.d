...
int ipfs_init();
void ipfs_destroy();
int ipfs_posix_fallocate(int fd, off_t offset, size_t len);
size_t ipfs_read(int fd, const(iovec)* iov, int iovcnt, bool has_offset, off_t offset);
size_t ipfs_write(int fd, const(iovec)* iov, int iovcnt, bool has_offset, off_t offset);
int ipfs_close(int fd);
void* ipfs_fopen(int fd, int flags);
int ipfs_fflush(int fd);
off_t ipfs_lseek(int fd, off_t offset, int nwhence);
int ipfs_ftruncate(int fd, off_t len);

/**
 * Whether two file descriptors are equal.
 */
pragma(inline, true) private bool fd_equal(int left, int right) {
 ...
 */
pragma(inline, true) private uint fd_hash(int fd) {
    return cast(uint)fd;
}
...
public import sgx_tprotected_fs;

enum SGX_ERROR_FILE_LOWEST_ERROR_ID = SGX_ERROR_FILE_BAD_STATUS;
enum SGX_ERROR_FILE_HIGHEST_ERROR_ID = SGX_ERROR_FILE_CLOSE_FAILED;

// The mapping between file descriptors and IPFS file pointers.
private HashMap* ipfs_file_list;

// Converts an SGX error code to a POSIX error code.
private __wasi_errno_t convert_sgx_errno(int error) {
...
private void* fd2file(int fd) {
    return bh_hash_map_find(ipfs_file_list, cast(void*)cast(intptr_t)fd);
}

private void ipfs_file_destroy(void* sgx_file) {
    sgx_fclose(sgx_file);
}

int ipfs_init() {
    ipfs_file_list =
        bh_hash_map_create(32, true, cast(HashFunc)fd_hash, cast(KeyEqualFunc)fd_equal,
                           null, cast(ValueDestroyFunc)ipfs_file_destroy);
...
size_t ipfs_read(int fd, const(iovec)* iov, int iovcnt, bool has_offset, off_t offset) {
    ...
    void* sgx_file = fd2file(fd);
    size_t read_result = void, number_of_read_bytes = 0;
    ...
    if (has_offset) {
        ...
        original_offset = cast(off_t)sgx_ftell(sgx_file);
...
size_t ipfs_write(int fd, const(iovec)* iov, int iovcnt, bool has_offset, off_t offset) {
    ...
    off_t original_offset = 0;
    void* sgx_file = fd2file(fd);
    size_t write_result = void, number_of_written_bytes = 0;
...
int ipfs_close(int fd) {
    ...
    if (!bh_hash_map_remove(ipfs_file_list, cast(void*)cast(intptr_t)fd, null,
                            &sgx_file)) {
        errno = EBADF;
        return -1;
    }
...
void* ipfs_fopen(int fd, int flags) {
    ...
    size_t ret = void;
    char[32] symbolic_path = void;
    ret =
        snprintf(symbolic_path.ptr, symbolic_path.sizeof, "/proc/self/fd/%d", fd);
    if (ret >= symbolic_path.sizeof) {
        errno = ENAMETOOLONG;
        return null;
    }
    ...
    char[PATH_MAX] real_path = 0;
    ret = readlink(symbolic_path.ptr, real_path.ptr, PATH_MAX - 1);
    if (ret == -1)
        return null;

    // Opening the file using the real path
    void* sgx_file = sgx_fopen_auto_key(real_path.ptr, mode);
    ...
    if (!bh_hash_map_insert(ipfs_file_list, cast(void*)cast(intptr_t)fd, sgx_file)) {
...
off_t ipfs_lseek(int fd, off_t offset, int nwhence) {
    ...
    if (offset == 0 && nwhence == SEEK_CUR) {
        long ftell_result = cast(off_t)sgx_ftell(sgx_file);
    ...
    if (fseek_result == 0) {
        new_offset = cast(__wasi_filesize_t)sgx_ftell(sgx_file);
        ...
        char zero = 0;
        long number_of_zeroes = offset - sgx_ftell(sgx_file);
