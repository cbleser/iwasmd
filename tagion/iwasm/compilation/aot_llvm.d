...
private bool create_argv_buf(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    if (((argv_buf_addr = LLVMBuildInBoundsGEP2(
              ...
              &argv_buf_offset, 1, "argv_buf_addr")) == 0)) {
...
private bool create_native_stack_bound(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    if (((stack_bound_addr = LLVMBuildInBoundsGEP2(
              comp_ctx.builder, OPQ_PTR_TYPE, func_ctx.exec_env,
              &stack_bound_offset, 1, "stack_bound_addr")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
...
private bool create_aux_stack_info(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    if (((aux_stack_bound_addr = LLVMBuildInBoundsGEP2(
              comp_ctx.builder, OPQ_PTR_TYPE, func_ctx.exec_env,
              &aux_stack_bound_offset, 1, "aux_stack_bound_addr")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    ...
    if (((aux_stack_bottom_addr = LLVMBuildInBoundsGEP2(
              comp_ctx.builder, OPQ_PTR_TYPE, func_ctx.exec_env,
              &aux_stack_bottom_offset, 1, "aux_stack_bottom_addr")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
...
private bool create_native_symbol(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    if (((native_symbol_addr = LLVMBuildInBoundsGEP2(
              comp_ctx.builder, OPQ_PTR_TYPE, func_ctx.exec_env,
              &native_symbol_offset, 1, "native_symbol_addr")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    ...
    if (((func_ctx.native_symbol =
              LLVMBuildLoad2(comp_ctx.builder, OPQ_PTR_TYPE,
                             native_symbol_addr, "native_symbol_tmp")) == 0)) {
        aot_set_last_error("llvm build bit cast failed");
        return false;
    }
...
static if (WASM_ENABLE_SHARED_MEMORY != 0) {
    ...
    if (is_shared_memory) {
        LLVMValueRef shared_mem_addr = void;
        offset = I32_CONST(AOTModuleInstance.memories.offsetof);
        if (!offset) {
        ...
        if (((shared_mem_addr = LLVMBuildInBoundsGEP2(
                  ...
                  "shared_mem_addr_offset")) == 0)) {
            aot_set_last_error("llvm build in bounds gep failed");
            return false;
        }
        if (((shared_mem_addr =
                  LLVMBuildBitCast(comp_ctx.builder, shared_mem_addr,
                                   int8_ptr_type, "shared_mem_addr_ptr")) == 0)) {
            aot_set_last_error("llvm build bit cast failed");
            return false;
        }
        /* aot_inst->memories[0] */
        if (((shared_mem_addr =
                  LLVMBuildLoad2(comp_ctx.builder, OPQ_PTR_TYPE,
                                 shared_mem_addr, "shared_mem_addr")) == 0)) {
            aot_set_last_error("llvm build load failed");
            return false;
        }
        if (((shared_mem_addr =
    ...
    if (mem_space_unchanged) {
        if (((func_ctx.mem_info[0].mem_base_addr = LLVMBuildLoad2(
                  ...
                  func_ctx.mem_info[0].mem_base_addr, "mem_base_addr")) == 0)) {
            aot_set_last_error("llvm build load failed");
            return false;
        }
        if (((func_ctx.mem_info[0].mem_cur_page_count_addr =
...
static if (WASM_ENABLE_SHARED_MEMORY != 0) {
    ...
    if (((func_ctx.mem_info[0].mem_bound_check_1byte =
              LLVMBuildInBoundsGEP2(comp_ctx.builder, INT8_TYPE, mem_info_base,
                                    &offset, 1, "bound_check_1byte_offset")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    ...
    if (mem_space_unchanged) {
        if (((func_ctx.mem_info[0].mem_bound_check_1byte = LLVMBuildLoad2(
                  comp_ctx.builder,
                  (comp_ctx.pointer_size == uint64.sizeof) ? I64_TYPE
                                                             : I32_TYPE,
    ...
    if (((func_ctx.mem_info[0].mem_bound_check_2bytes =
              LLVMBuildInBoundsGEP2(comp_ctx.builder, INT8_TYPE, mem_info_base,
                                    &offset, 1, "bound_check_2bytes_offset")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    ...
    if (((func_ctx.mem_info[0].mem_bound_check_4bytes =
              LLVMBuildInBoundsGEP2(comp_ctx.builder, INT8_TYPE, mem_info_base,
                                    &offset, 1, "bound_check_4bytes_offset")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    ...
    if (((func_ctx.mem_info[0].mem_bound_check_8bytes =
              LLVMBuildInBoundsGEP2(comp_ctx.builder, INT8_TYPE, mem_info_base,
                                    &offset, 1, "bound_check_8bytes_offset")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
...
static if (WASM_ENABLE_DEBUG_AOT != 0) {
    ...
    if (((aot_inst_addr = LLVMBuildInBoundsGEP2(
              comp_ctx.builder, OPQ_PTR_TYPE, func_ctx.exec_env,
              &aot_inst_offset, 1, "aot_inst_addr")) == 0)) {
        aot_set_last_error("llvm build in bounds gep failed");
        goto fail;
    }
...
enum string CREATE_I1_CONST(string name, string value) = `                                       \
    if (!(consts->i1_##name =                                              \
...
enum string CREATE_I32_CONST(string name, string value) = `                                \
    if (!(consts->i32_##name = LLVMConstInt(I32_TYPE, value, true))) \
        return false;`;

    CREATE_I32_CONST(min, cast(uint)INT32_MIN)
...
enum string CREATE_VEC_ZERO_MASK(string slot) = `                                       \
    {                                                                    \
...
static if (WASM_ENABLE_DEBUG_AOT != 0) {
    if (((comp_ctx.debug_builder = LLVMCreateDIBuilder(comp_ctx.module_)) == 0)) {
        aot_set_last_error("create LLVM Debug Infor builder failed.");
        goto fail;
    }

    LLVMAddModuleFlag(
        comp_ctx.module_, LLVMModuleFlagBehaviorWarning, "Debug Info Version",
...
static if (HasVersion!"OSX" || HasVersion!"OSX") {
        if (!abi) {
...
static if (LLVM_VERSION_MAJOR >= 14) {
    func_type =
        LLVMFunctionType(ret_type, param_types, cast(uint)param_count, false);
}

    /* Call the LLVM intrinsic function */
    if (((ret = LLVMBuildCall2(comp_ctx.builder, func_type, func, param_values,
                               cast(uint)param_count, "call")) == 0)) {
...
LLVMValueRef aot_load_const_from_table(AOTCompContext* comp_ctx, LLVMValueRef base, const(WASMValue)* value, ubyte value_type) {
    ...
    if (((const_addr =
              LLVMBuildInBoundsGEP2(comp_ctx.builder, OPQ_PTR_TYPE, base,
                                    &const_index, 1, "const_addr_tmp")) == 0)) {
...
static if (LLVM_VERSION_MAJOR < 14) {
enum string LLVMBuildLoad2(string builder, string type, string value, string name) = ` \
    LLVMBuildLoad(builder, value, name)`;

enum string LLVMBuildCall2(string builder, string type, string func, string args, string num_args, string name) = ` \
    LLVMBuildCall(builder, func, args, num_args, name)`;

enum string LLVMBuildInBoundsGEP2(string builder, string type, string ptr, string indices, string num_indices, string name) = ` \
    LLVMBuildInBoundsGEP(builder, ptr, indices, num_indices, name)`;
} else {
/* Opaque pointer type */
enum OPQ_PTR_TYPE = INT8_PTR_TYPE;
}
