...
enum string LLVM_BUILD_ICMP(string op, string left, string right, string res, string name) = `                           \
    do {                                                                      \
        if (!(res =                                                           \
...
enum string LLVM_BUILD_OP(string Op, string left, string right, string res, string name, string err_ret) = `                  \
    do {                                                                    \
...
enum string LLVM_BUILD_OP_OR_INTRINSIC(string Op, string left, string right, string res, string intrinsic, string name, \
                                   string err_ret) = `                               \
    do {                                                                  \
...
static if (LLVM_VERSION_NUMBER >= 12) {
...
enum string CHECK_INT_OVERFLOW(string type) = `                                           \
    do {                                                                   \
...
private bool aot_compile_int_bit_count(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, BitCountType type, bool is_i32) {
    ...
    if (type < POP_CNT32)
        DEF_INT_UNARY_OP(aot_call_llvm_intrinsic(
                             ...
                             ret_type, param_types.ptr, 2, operand, zero_undef),
                         null);
    else
        DEF_INT_UNARY_OP(aot_call_llvm_intrinsic(
                             comp_ctx, func_ctx, bit_cnt_llvm_intrinsic[type],
                             ret_type, param_types.ptr, 1, operand),
                         null);
...
private bool compile_float_copysign(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, bool is_f32) {
    ...
    DEF_FP_BINARY_OP(aot_call_llvm_intrinsic(
                         ...
                         ret_type, param_types.ptr, 2, left, right),
                     null);
    return true;
