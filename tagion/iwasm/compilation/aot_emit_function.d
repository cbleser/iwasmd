...
private bool call_aot_alloc_frame_func(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, LLVMValueRef func_idx) {
    LLVMValueRef[2] param_values = void; LLVMValueRef ret_value = void, value = void, func = void;
    LLVMTypeRef[2] param_types = void; LLVMTypeRef ret_type = void, func_type = void, func_ptr_type = void;
    LLVMBasicBlockRef block_curr = LLVMGetInsertBlock(comp_ctx.builder);
    LLVMBasicBlockRef frame_alloc_fail = void, frame_alloc_success = void;
    AOTFuncType* aot_func_type = func_ctx.aot_func.func_type;
    ...
    if (((ret_value =
              LLVMBuildCall2(comp_ctx.builder, func_type, func, param_values.ptr,
                             2, "call_aot_alloc_frame")) == 0)) {
        aot_set_last_error("llvm build call failed.");
        return false;
    }

    if (((ret_value = LLVMBuildICmp(comp_ctx.builder, LLVMIntUGT, ret_value,
                                    I8_ZERO, "frame_alloc_ret")) == 0)) {
        aot_set_last_error("llvm build icmp failed.");
        return false;
    }
...
private bool call_aot_free_frame_func(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    LLVMValueRef[1] param_values = void; LLVMValueRef ret_value = void, value = void, func = void;
    LLVMTypeRef[1] param_types = void; LLVMTypeRef ret_type = void, func_type = void, func_ptr_type = void;
    ...
    if (((ret_value = LLVMBuildCall2(comp_ctx.builder, func_type, func,
                                     param_values.ptr, 1, "call_aot_free_frame")) == 0)) {
        aot_set_last_error("llvm build call failed.");
        return false;
    }
...
static if ((WASM_ENABLE_DUMP_CALL_STACK != 0) || (WASM_ENABLE_PERF_PROFILING != 0)) {
    if (comp_ctx.enable_aux_stack_frame) {
        LLVMValueRef func_idx_const = void;

        if (((func_idx_const = I32_CONST(func_idx)) == 0)) {
            aot_set_last_error("llvm build const failed.");
            return false;
        }
...
static if (LLVM_VERSION_MAJOR >= 14) {
        LLVMTypeRef llvm_func_type = void;
}
...
static if (!(WASM_ENABLE_FAST_JIT != 0 && WASM_ENABLE_LAZY_JIT != 0)) {
                    ...
                    LLVMValueRef func_ptr = void, func_idx_const = void;
                    LLVMTypeRef func_ptr_type = void;
...
} /* end of !(WASM_ENABLE_FAST_JIT != 0 && WASM_ENABLE_LAZY_JIT != 0) */
                }
            }
        }
...
bool aot_compile_op_call_indirect(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, uint type_idx, uint tbl_idx) {
    ...
    total_size = sizeof(LLVMTypeRef) * cast(ulong)total_param_count;
    if (total_size >= UINT32_MAX
        || ((param_types = wasm_runtime_malloc(cast(uint)total_size)) == 0)) {
        aot_set_last_error("allocate memory failed.");
        goto fail;
    ...
    /* Allocate memory for parameters */
    total_size = sizeof(LLVMValueRef) * cast(ulong)total_param_count;
    if (total_size >= UINT32_MAX
        || ((param_values = wasm_runtime_malloc(cast(uint)total_size)) == 0)) {
        aot_set_last_error("allocate memory failed.");
        goto fail;
...
static if ((WASM_ENABLE_DUMP_CALL_STACK != 0) || (WASM_ENABLE_PERF_PROFILING != 0)) {
    ...
    if (func_result_count > 0) {
        total_size = sizeof(LLVMValueRef) * cast(ulong)func_result_count;
        if (total_size >= UINT32_MAX
