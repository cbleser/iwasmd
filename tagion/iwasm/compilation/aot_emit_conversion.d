...
private bool trunc_float_to_int(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, LLVMValueRef operand, LLVMTypeRef src_type, LLVMTypeRef dest_type, LLVMValueRef min_value, LLVMValueRef max_value, char* name, bool sign) {
    ...
    LLVMValueRef is_less = void, is_greater = void, res = void;

    if (comp_ctx.disable_llvm_intrinsics
        && aot_intrinsic_check_capability(
            comp_ctx, src_type == F32_TYPE ? "f32_cmp" : "f64_cmp")) {
        LLVMTypeRef[3] param_types = void;
        ...
        res = aot_call_llvm_intrinsic(
            ...
            I32_TYPE, param_types.ptr, 3, opcond, operand, operand);
        if (!res) {
            goto fail;
        }
    ...
    if (((check_nan_succ = LLVMAppendBasicBlockInContext(
              comp_ctx.context, func_ctx.func, "check_nan_succ")) == 0)) {
    ...
    if (comp_ctx.disable_llvm_intrinsics
        ...
        is_less = aot_call_llvm_intrinsic(
            comp_ctx, func_ctx, src_type == F32_TYPE ? "f32_cmp" : "f64_cmp",
            I32_TYPE, param_types.ptr, 3, opcond, operand, min_value);
        if (!is_less) {
            goto fail;
        }
        is_less =
    ...
    if (comp_ctx.disable_llvm_intrinsics
        ...
        is_greater = aot_call_llvm_intrinsic(
            comp_ctx, func_ctx, src_type == F32_TYPE ? "f32_cmp" : "f64_cmp",
            I32_TYPE, param_types.ptr, 3, opcond, operand, max_value);
        if (!is_greater) {
            goto fail;
        }
    ...
    if (((check_overflow_succ = LLVMAppendBasicBlockInContext(
              comp_ctx.context, func_ctx.func, "check_overflow_succ")) == 0)) {
        aot_set_last_error("llvm add basic block failed.");
        goto fail;
    }
...
private bool trunc_sat_float_to_int(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, LLVMValueRef operand, LLVMTypeRef src_type, LLVMTypeRef dest_type, LLVMValueRef min_value, LLVMValueRef max_value, char* name, bool sign) {
    LLVMBasicBlockRef check_nan_succ = void, check_less_succ = void, check_greater_succ = void;
    LLVMBasicBlockRef is_nan_block = void, is_less_block = void, is_greater_block = void, res_block = void;
    LLVMValueRef is_less = void, is_greater = void, res = void, phi = void;
    LLVMValueRef zero = (dest_type == I32_TYPE) ? I32_ZERO : I64_ZERO;
    LLVMValueRef vmin = void, vmax = void;
    ...
    if (comp_ctx.disable_llvm_intrinsics
        ...
        char[128] intrinsic = void;

        /* Integer width is always 32 or 64 here. */

        snprintf(intrinsic.ptr, intrinsic.sizeof, "i%d_trunc_f%d_%c",
                 LLVMGetIntTypeWidth(dest_type),
                 LLVMGetTypeKind(src_type) == LLVMFloatTypeKind ? 32 : 64,
                 sign ? 's' : 'u');

        if (comp_ctx.disable_llvm_intrinsics
            && aot_intrinsic_check_capability(comp_ctx, intrinsic.ptr)) {
            res = aot_call_llvm_intrinsic(comp_ctx, func_ctx, intrinsic.ptr,
...
bool aot_compile_op_i32_wrap_i64(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    LLVMValueRef value = void, res = void;
...
bool aot_compile_op_i32_trunc_f32(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, bool sign, bool saturating) {
    ...
    LLVMValueRef min_value = void, max_value = void;
...
bool aot_compile_op_i64_extend_i64(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx, byte bitwidth) {
    LLVMValueRef value = void, res = void, cast_value = null;
    ...
    if (comp_ctx.disable_llvm_intrinsics
        ...
        res = aot_call_llvm_intrinsic(comp_ctx, func_ctx,
                                      sign ? "f32_convert_i32_s"
                                           : "f32_convert_i32_u",
                                      F32_TYPE, param_types.ptr, 1, value);
    }
    else {
        if (sign)
    ...
    if (comp_ctx.disable_llvm_intrinsics
        ...
        res = aot_call_llvm_intrinsic(comp_ctx, func_ctx,
                                      sign ? "f64_convert_i32_s"
                                           : "f64_convert_i32_u",
                                      F64_TYPE, param_types.ptr, 1, value);
    }
    else {
        if (sign)
...
bool aot_compile_op_i64_reinterpret_f64(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    POP_F64(value);
    if (((value =
              LLVMBuildBitCast(comp_ctx.builder, value, I64_TYPE, "i64")) == 0)) {
...
bool aot_compile_op_i32_reinterpret_f32(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    POP_F32(value);
    if (((value =
              LLVMBuildBitCast(comp_ctx.builder, value, I32_TYPE, "i32")) == 0)) {
        aot_set_last_error("llvm build fp to si failed.");
        return false;
    }
    PUSH_I32(value);
...
bool aot_compile_op_f64_reinterpret_i64(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    POP_I64(value);
    if (((value =
              LLVMBuildBitCast(comp_ctx.builder, value, F64_TYPE, "f64")) == 0)) {
...
bool aot_compile_op_f32_reinterpret_i32(AOTCompContext* comp_ctx, AOTFuncContext* func_ctx) {
    ...
    POP_I32(value);
    if (((value =
              LLVMBuildBitCast(comp_ctx.builder, value, F32_TYPE, "f32")) == 0)) {
        aot_set_last_error("llvm build si to fp failed.");
        return false;
    }
    PUSH_F32(value);
