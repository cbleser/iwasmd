...
void hmu_init_prefix_and_suffix(hmu_t* hmu, gc_size_t tot_size, const(char)* file_name, int line_no) {
    ...
    prefix = cast(gc_object_prefix_t*)(hmu + 1);
    suffix =
        cast(gc_object_suffix_t*)(cast(gc_uint8*)hmu + tot_size - OBJ_SUFFIX_SIZE);
...
void hmu_verify(void* vheap, hmu_t* hmu) {
    ...
    prefix = cast(gc_object_prefix_t*)(hmu + 1);
    size = prefix.size;
    suffix = cast(gc_object_suffix_t*)(cast(gc_uint8*)hmu + size - OBJ_SUFFIX_SIZE);
