...
private SemaphoreHandle_t thread_data_lock;
...
private os_thread_data* thread_data_list_lookup(TaskHandle_t handle) {
    xSemaphoreTake(thread_data_lock, portMAX_DELAY);
    if (thread_data_list) {
        os_thread_data* p = thread_data_list;
        while (p) {
...
int os_thread_sys_init() {
    ...
    if (((thread_data_lock = xSemaphoreCreateMutex()) == 0))
        return BHT_ERROR;

    /* Initialize supervisor thread data */
    memset(&supervisor_thread_data, 0, supervisor_thread_data.sizeof);

    if (((supervisor_thread_data.wait_node.sem = xSemaphoreCreateBinary()) == 0)) {
...
private void os_thread_cleanup() {
    ...
    os_thread_wait_list thread_wait_list = void;
    SemaphoreHandle_t wait_list_lock = void;
    SemaphoreHandle_t wait_node_sem = void;
...
int os_thread_create_with_prio(korp_tid* p_tid, thread_start_routine_t start, void* arg, uint stack_size, int prio) {
    ...
    if (((thread_data.wait_node.sem = xSemaphoreCreateBinary()) == 0))
        goto fail1;

    if (((thread_data.wait_list_lock = xSemaphoreCreateMutex()) == 0))
        goto fail2;

    snprintf(thread_name.ptr, thread_name.sizeof, "%s%d", "wasm-thread-",
             ++thread_name_index);

    /* Create the thread */
    if (pdPASS
        != xTaskCreate(&os_thread_wrapper, thread_name.ptr, stack_size / 4,
...
int os_mutex_init(korp_mutex* mutex) {
    SemaphoreHandle_t semaphore = void;

    if (((semaphore = xSemaphoreCreateMutex()) == 0))
        return BHT_ERROR;
    mutex.sem = semaphore;
...
int os_recursive_mutex_init(korp_mutex* mutex) {
    ...
    if (((semaphore = xSemaphoreCreateRecursiveMutex()) == 0))
        return BHT_ERROR;
    mutex.sem = semaphore;
...
int os_cond_init(korp_cond* cond) {
    if (((cond.wait_list_lock = xSemaphoreCreateMutex()) == 0))
        return BHT_ERROR;
