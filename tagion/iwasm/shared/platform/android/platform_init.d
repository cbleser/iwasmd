...
enum string API_NOT_SUPPORT_ERROR(string API, string VERSION) = `                                   \
...
static if (__ANDROID_API__ < 19) {

int futimens(int __dir_fd, const(timespec)[2] __times) {
...
int accept4(int __fd, sockaddr* __addr, socklen_t* __addr_length, int __flags) {
...
int epoll_pwait(int epfd, epoll_event* events, int maxevents, int timeout, const(sigset_t)* sigmask) {
...
static if (__ANDROID_API__ < 23) {

c_long telldir(DIR* __dir) {
    API_NOT_SUPPORT_ERROR(telldir, 23);
    return -1;
}

void seekdir(DIR* __dir, c_long __location) {
    API_NOT_SUPPORT_ERROR(seekdir, 23);
}

}

static if (__ANDROID_API__ < 24) {

ssize_t preadv(int __fd, const(iovec)* __iov, int __count, off_t __offset) {
    API_NOT_SUPPORT_ERROR(preadv, 24);
    return -1;
}

ssize_t pwritev(int __fd, const(iovec)* __iov, int __count, off_t __offset) {
